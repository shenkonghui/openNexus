...
- **Task Assistant**: Multi-task orchestration with git worktree isolation, goal auto-continuation, and a multi-task grid mode for live parallel output. Completed tasks automatically collapse into compact cards in the rightmost column, keeping the workspace focused on active tasks. The global workspace selector supports an "All Workspaces" mode: tasks from all workspaces are aggregated (list and multi-task grid mixed, each card tagged with its source workspace), with cross-workspace start/stop/delete and task creation (pick a target workspace first).
