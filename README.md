...
- **任务管理**：基于 git worktree 隔离的多任务并发编排，支持 goal 自动续轮；在任务管理对话中通过 `@task` 引用把消息直发到指定任务会话，已完成的任务折叠为紧凑卡片收纳到「已完成」分栏；未开始的任务支持编辑与删除，「全部启动」需二次确认；已归档分栏默认隐藏（工具栏「更多」菜单可开启），并支持在设置中配置已完成任务超过 N 天（默认 7 天）无交互后自动归档。全局工作区选择器支持「全部工作区」：聚合展示所有工作区的任务（任务卡片标注来源工作区），可跨工作区启停/删除任务并新建任务（先选目标工作区），聚合视图不展示任务助手对话；侧边栏工作区分组支持上下拖动排序（本地持久化）
